...
def vtab_active_binds(task: AgentSSLTaskConfig) -> tuple[dict[str, str], dict[str, str]]:
    experiment_dir = Path(task.experiment_dir).resolve()
    workspace_dir = experiment_dir / "workspace"
    program_path = experiment_dir / "program.py"
    ...
    task_root = Path(task.ssl_dir).resolve() / task.benchmark

    ro_binds = {
        str(Path(task.task_dir).resolve()): "/task",
        str(Path(task.eval_dir).resolve()): "/eval",
        str(task_root / "eval" / "shared" / "assets"): "/assets",
        str(task_root / "eval" / "shared" / "core"): "/eval/src",
        str(program_path): "/program.py",
    }
...
def is_vtab_active_noprior_task(task: AgentSSLTaskConfig) -> bool:
    return Path(task.ssl_dir).resolve().name == "tasks_active_noprior"


def vtab_active_noprior_binds(task: AgentSSLTaskConfig) -> tuple[dict[str, str], dict[str, str]]:
    experiment_dir = Path(task.experiment_dir).resolve()
    workspace_dir = experiment_dir / "workspace"
    program_path = experiment_dir / "program.py"
